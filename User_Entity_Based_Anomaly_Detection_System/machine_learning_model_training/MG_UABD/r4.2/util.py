# -------------------------------------------------------------------------------------
# 'content' 컬럼에서 파생되는 feature를 제거하기 위해 원본 util.py를 수정하였습니다.
# -------------------------------------------------------------------------------------
...
def add_action_thisweek(act, columns, lines, act_handles, week_index, stop, firstdate, dname = 'r5.2'):
    ...
    while True:
        ...
        if dname in ['r6.1','r6.2'] and act in ['email', 'file','http'] and '"' in lines[act]:
            ...
            # 'r4.2'의 경우 이 코드가 실행됩니다.
            tmp = lines[act].split(',')
            
            # 유일하게 추가된 수정 부분
            # email, file, http 로그의 경우, 파싱된 데이터 리스트의 마지막 요소를 버립니다.
            if act in ['email', 'file', 'http']:
                tmp = tmp[:-1]

        # '\n' 문자가 포함된 경우 제거 (원본 코드에는 없었지만 안정성을 위해 추가)
        if tmp and '\n' in tmp[-1]:
            tmp[-1] = tmp[-1].replace('\n', '')
...
def combine_by_timerange_pandas(dname='r4.2'):
    ...
    firstline = subprocess.check_output(['head', '-2', 'http.csv']).decode().split('\n')[1]
    ...
    with tqdm(total=TOTAL_WEEKS, desc="Weekly combine", unit="week") as pbar:
        while sum(stop.values()) < len(allacts) and week_index < TOTAL_WEEKS:
            week_chunks = []
            ...
            for act in allacts:
                ...
                if act == 'email':
                    if dname in ['r4.1','r4.2']:
                        columns = ['id','date','user','pc','to','cc','bcc','from','size','#att']
                    else:  # r6.*, r5.*
                        columns = ['id','date','user','pc','to','cc','bcc','from','activity','size','att']
                elif act == 'logon':
                    ...
                    if dname in ['r6.1','r6.2']:
                        columns = ['id','date','user','pc','url/fname','activity']
                    else:
                        columns = ['id','date','user','pc','url/fname']
                elif act == 'file':
                    if dname in ['r4.1','r4.2']:
                        columns = ['id','date','user','pc','url/fname']
                    else:
                        columns = ['id','date','user','pc','url/fname','activity','to','from']
                # ---------------------

                df = add_action_thisweek(act, columns, lines, act_handles, week_index, stop, firstdate, dname=dname)
                if not df.empty:
                    week_chunks.append(df)

            thisweekdf = pd.concat(week_chunks, ignore_index=False, sort=False) if week_chunks else pd.DataFrame()

            if not thisweekdf.empty:
                thisweekdf['date'] = pd.to_datetime(thisweekdf['date'], format="%m/%d/%Y %H:%M:%S", errors='coerce')

            thisweekdf.to_pickle(f"DataByWeek/{week_index}.pickle")

            # 진행도 업데이트
            pbar.update(1)
            pbar.set_postfix(week=week_index, rows=len(thisweekdf))

            week_index += 1

    for f in act_handles.values():
        try:
            f.close()
        except Exception:
            pass
...
def getuserlist(dname = 'r4.2', psycho = True):
    allfiles =  [
        os.path.join('LDAP', f)
        for f in os.listdir('LDAP')
        if f.lower().endswith('.csv') and os.path.isfile(os.path.join('LDAP', f))
    ]
    alluser = {}
    alreadyFired = []
    ...
    print("Step 2-2. LDAP 파일 처리 완료")

    print("Step 2-3. 개인 PC 정보 수집 시작")
    df['pc'] = None
    for i in df.index:
...
def get_mal_userdata(data = 'r4.2', usersdf = None):
    ...
    try:
        ...
        raise Exception("answers/insiders.csv not found. Please download the answers dataset from the CERT website.")
    ...
    if type(usersdf) != pd.core.frame.DataFrame:
        usersdf = getuserlist(data, False)
    usersdf['malscene']=0
    usersdf['mstart'] = None
    usersdf['mend'] = None
    ...
    try:
        ...
        print(f"Error processing malicious user data: {e}")
        return None
    return usersdf
...
def http_process(act, data = 'r4.2'): 
    ...
    if data in ['r6.1','r6.2']:
        ...
        http_act = http_act_dict.get(act['activity'].lower(), 0)
       	return [r, url_len, url_depth, http_act]
    else:
...
def process_week_num(week, users, userlist = 'all', data = 'r4.2'):
    try:
        ...
        if data in ['r4.2','r4.1']: n_cols = 21
        ...
        for u in userlist:
            ...
            # 현재 사용자 u가 이번 주차에 악성 시나리오를 가짐.
            # 악성 시나리오 번호: users.loc[u].malscene
        ...
        if data in ['r4.1','r4.2']:
            ...
            file_feature_names = ['file_type', 'disk', 'file_depth']
            http_feature_names = ['http_type', 'url_len','url_depth']
            email_feature_names = ['n_des', 'n_atts', 'Xemail', 'n_exdes', 'n_bccdes', 'exbccmail', 'email_size']
        elif data in ['r5.2','r5.1', 'r6.2','r6.1']:
...
def f_calc(ud, mode = 'week', data = 'r4.2'):
    ...
    (all_filef, all_filef_names) = f_calc_subfeatures(ud[ud['act']==7], 'file', 'file_type', [1,2,3,4,5,6], \
            ['otherf','compf','phof','docf','txtf','exef'], ['file_depth'], file_countonlyf)
    
    if mode == 'day':
        (workhourfilef, workhourfilef_names) = f_calc_subfeatures(ud[(ud['act']==7) & ((ud['time'] ==1) | (ud['time'] ==3))], 'workhourfile', 'file_type', [1,2,3,4,5,6], ['otherf','compf','phof','docf','txtf','exef'], ['file_depth'], file_countonlyf)
        (afterhourfilef, afterhourfilef_names) = f_calc_subfeatures(ud[(ud['act']==7) & ((ud['time'] ==2) | (ud['time'] ==4))], 'afterhourfile', 'file_type', [1,2,3,4,5,6], ['otherf','compf','phof','docf','txtf','exef'], ['file_depth'], file_countonlyf)
    elif mode == 'week':
        (workhourfilef, workhourfilef_names) = f_calc_subfeatures(ud[(ud['act']==7) & (ud['time'] ==1)], 'workhourfile', 'file_type', [1,2,3,4,5,6], ['otherf','compf','phof','docf','txtf','exef'], [ 'file_depth'], file_countonlyf)
        (afterhourfilef, afterhourfilef_names) = f_calc_subfeatures(ud[(ud['act']==7) & (ud['time'] ==2)], 'afterhourfile', 'file_type', [1,2,3,4,5,6], ['otherf','compf','phof','docf','txtf','exef'], [ 'file_depth'], file_countonlyf)
        (weekendfilef, weekendfilef_names) = f_calc_subfeatures(ud[(ud['act']==7) & (ud['time'] >= 3)], 'weekendfile', 'file_type', [1,2,3,4,5,6], ['otherf','compf','phof','docf','txtf','exef'], [ 'file_depth'], file_countonlyf)
    ...
    (all_httpf, all_httpf_names) = f_calc_subfeatures(ud[ud['act']==5], 'http', 'http_type', [1,2,3,4,5,6], \
            ['otherf','socnetf','cloudf','jobf','leakf','hackf'], ['url_len', 'url_depth'], http_count_subf)
...
def to_csv(week, mode, data, ul, uf_dict, list_uf, subsession_mode = {}):
    """
    mode 에 따라 사용자 단위로 집계한 피처 테이블을 만듬.
    day: 사용자 x 해당 주의 각 날짜 1행
    week: 사용자 x 해당 주 1행
    session: 사용자 x 로그온-오프 세션 1행 

    cols2a: 각 column 별 메타데이터 
    참조: ['O', 'C', 'E', 'A', 'N'] 행은 PSYCOMETRIC이 포함되는 경우에 한하므로, 제외. 
    """
    ...
    if mode == 'session': 
        ...
        cols2a = ['starttime', 'endtime','user', 'sessionid', 'day', 'week', 'pc', 'isworkhour', 'isafterhour','isweekend', 
                  ...
                  'ses_end'] + list_uf + ['ITAdmin']
    elif mode == 'day': 
        cols2a = ['starttime', 'endtime','user', 'day', 'week', 'isweekday','isweekend'] + list_uf + ['ITAdmin']
    else: cols2a = ['starttime', 'endtime','user','week'] + list_uf + ['ITAdmin']
    cols2b = ['insider']        
        ...
        if v in usnlist:
            ...
            if mode == 'week':
                ...
                if len(uactw) > 0:
                    ...
                    i_fnames = tmp[3]
                    towrite_list.append([starttime, endtime, v, week] + (uw.loc[v, list_uf + ['ITAdmin']]).tolist() + tmp[2] + [ tmp[4]])
            ...
            if mode == 'day':
                ...
                for d in days:
                    ...
                    if len(ud) > 0:
                        tmp = f_calc(ud, mode, data)
                        i_fnames = tmp[3]
                        towrite_list.append([starttime, endtime, v, d, week, isweekday, isweekend] + (uw.loc[v, list_uf + ['ITAdmin'] ]).tolist() + tmp[2] + [ tmp[4]])
